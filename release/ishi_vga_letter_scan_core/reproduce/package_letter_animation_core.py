#!/usr/bin/env python3
"""Publish the checked letter-animation core and the programmed FPGA image."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--evidence',type=Path,required=True);a=ap.parse_args();verify()
    ...
    for name in ['vga_letter_animation.fs','program.log','preprogram_verification.json','sha256.json','commands.json','timing.json','synth.log','pnr.log','pack.log']:copy(fpga/name,'fpga/'+name)
    program=(fpga/'program.log').read_bytes();assert b'100.00%' in program and b'DONE' in program
    ...
    frames[0].save(out/'animation.gif',save_all=True,append_images=frames[1:],duration=[270,260,270,270,260],loop=0,disposal=2,optimize=False)
